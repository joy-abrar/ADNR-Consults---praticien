...
class Calendar 
{
  ...
  function save ($start, $end, $txt, $color, $id=null) 
  {
    ...
    if ($id==null) 
    {
      $sql = "INSERT INTO `events` (`evt_start`, `evt_end`, `evt_text`, `evt_color`, `praticienId`) VALUES (?,?,?,?,?)";
      $data = [$start, $end, $txt, $color,$userId];
    } 
    else 
    {
  ...
  function get ($month, $year) 
  {
    ...
    // (F2) GET EVENTS
    if (!$this->exec(
      "SELECT * FROM `events` WHERE ( (`evt_start` BETWEEN ? AND ?) OR (`evt_end` BETWEEN ? AND ?) OR (`evt_start` <= ? AND `evt_end` >= ?)) AND (`praticienId` = ?)", 
      [$dayFirst, $dayLast, $dayFirst, $dayLast, $dayFirst, $dayLast, $userId]
    )) 
      { 
        return false; 
      }
...
define("DB_NAME", "c1AdnCons2022bdd");
define("DB_CHARSET", "utf8");
define("DB_USER", "c1ConsAdn2022");
define("DB_PASSWORD", "aqPZQq#8Zs");
