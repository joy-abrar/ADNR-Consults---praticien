<?php
	...
	<html>
		<head>
			...
			<!------- END LOADING SCREEN ------- -->
			<title>ADNR Consults - Mes clients</title>
		</head>
		<body id="body">
			<!------------ LOADING SCREEN ------------->
			...
			<div id="mainAppearence">
				...
				<h2 id="testTitle">MES CLIENTS</h2>
				<div id="seventhBloc" style="visibility: visible;">
					...
					<?php
						...
						else
						{
							...
							while ($rows2 = $connectPraticienToClient -> fetch()) 
								{
									$clientId = $rows2['clientId'];

									$getThisClientDetailsForMe = $userManager -> getThisClientDetailsForMe($clientId);

									while ($rows3 = $getThisClientDetailsForMe -> fetch()) 
									{
										$clientDateOfBirth = strtotime($rows3['dob']);
										$clientDateOfBirth = date("d-m-Y" , $clientDateOfBirth);
					?>
										<center id="clientListBloc">
											<h3><?= base64_decode( $rows3['firstname'] ); echo "&nbsp;"; echo base64_decode($rows3['lastname']) ?></h3>
											<h3>Date de naissance : <?= $clientDateOfBirth ?></h3>
											<h4>Age :
												<?php 
											
													$clientDateOfBirth = $rows3['dob'];
						  							$today = date("Y-m-d");
									  	 		...
									  	 		?>
									  	 	 Ans</h4>
									  	 	 <form method="POST" action="index.php?action=getClientDetailsFromUserLists&id=sdkjfh324d654vjkh564dfjkf56462djk<?= $rows3['id'] ?>gfjkhfgdfg546dfg54bhk5546465">
